...
$execute {
    ...
    // resetLevelVariables releases both players through one shared reset path.
    // Keep those releases from destroying the per-player buffered state.
    // The UILayer::handleKeypress patch was removed: it was a broad NOP patch in
    // the shared keyboard path and could crash with Geode keybind hooks installed.
...
// Helper to get the state reference from a layer (no map!)
...
class $modify(DualInputBaseLayer, GJBaseGameLayer)
{
    bool init()
    {
        if (!GJBaseGameLayer::init())
            return false;
        // Allocate the state once and attach it to the node.
    ...
    void onExit()
    {
        // Free the state that was allocated in init()
...
// fixes ingame bug where before respawning or starting the level, buffered inputs on either player 1 or player 2 gets sent to both players once the level starts (happens with both hold and release) still happens
// the correction should result in indpendent buffering inputs/releases of player 1 and player 2